...
(function() {
  ...
  Track = (function(superClass) {
      ...
      return this.Spotify.get(this.trackId, (function(_this) {
        return function(err, track) {
          if (err) {
            ...
            _this.track = track;
            return _this.Spotify.recurseAlternatives(track, "CA", (function(_err, newTrack) {
              if(_err) {
    ...
    Track.prototype.createDirs = function() {
      ...
      var file_name = fixFilename(this.track.name.replace(/\//g, ' - ')) + '.mp3';

      var artist_var = "_ARTIST_";
      if(globalDownloadType == "album") {
        if (this.track.album.artist[0].name != undefined)
          artist_var = this.track.album.artist[0].name;
      }
      else {
      ...
      if(this.isMakeFolder) {
        filepath = dir + '/' + fixFilename(artist_var.replace(/\//g, ' - ')) + ' - ' + file_name;
      }
      else {
        artistpath = dir + '/' + fixFilename(artist_var.replace(/\//g, ' - ')) + '/';
        albumpath = artistpath + fixFilename(this.track.album.name.replace(/\//g, ' - ')) + ' [' + this.track.album.date.year + ']/';
        filepath = albumpath + file_name;
      }
      if (fs.existsSync(filepath)) {
        stats = fs.statSync(filepath);
        if (stats.size !== 0) {
          Log("Already Downloaded: " + this.track.artist[0].name + " - " + this.track.name);
          return this.cb();
        }
      }
      if(this.isMakeFolder) {
        ...
        return function() {
          ...
          return _this.track.play().pipe(out).on('finish', function() {
            if(failed) {
              ...
              console.log((" - - " + (err.toString()) + " ...  { ratelimited }").red);
              return null;
            }
    ...
    Track.prototype.writeMetaData = function(filepath) {
      var artist_names = "";
      for(var _key in this.track.artist) {
        var _art = this.track.artist[_key].name;
        artist_names = _art + "/" + artist_names;
      }
      artist_names = artist_names.slice(0, -1);
  ...
  Downloader = (function(superClass) {
    ...
    function Downloader(username, password, directory, type, arguments) {
    ...
    Downloader.prototype.run = function() {
      ...
      return async.series([this.attemptLogin, this.processUri, this.processTracks], (function(_this) {
    ...
    Downloader.prototype.processUri = function(cb) {
      Log('Processing URI');
      if(this.type == "album") {
        return this.Spotify.getAlbum(this.uri, (function(_this) {
          return function(err, albumData) {
            if (err) {
              return Error("Album data error... " + err);
            }
            Log("Got Album: " + albumData.name);
            //Log("Got Album: " + albumData.attributes.name);
            if (_this.makeFolder) {
              _this.dir = _this.directory + "/" + albumData.name.replace(/\//g, '-') + '/';
            }
            _this.Tracks = lodash.map(albumData.tracks.items, function(item) {
              return item.uri;
            });
            return typeof cb === "function" ? cb() : void 0;
          };
        ...
        return this.Spotify.playlist(this.uri, 0, 9001, (function(_this) {
